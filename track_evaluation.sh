    ...
    do
        ...
        do
            ...
            do
                ...
                echo python main.py -f IMAGE/$date -l LABEL/$date --$param $i --out aicup_ts/labels/"${date}_${param}_${i}" --visualize $visualize --cam $cam --finetune True --model $model
                python main.py -f IMAGE/"${date}" -l LABEL/"${date}" --$param $i --out aicup_ts/labels/"${date}_${param}_${i}" --visualize $visualize --cam $cam --finetune True --model $model
                ...
                # Evaluate the result.
                echo python tools/evaluate.py --gt_dir MOT15/aicup_gt/$date --ts_dir MOT15/aicup_ts/"${date}_${param}_${i}"/"$model" --mode single_cam --cam $cam
                python tools/evaluate.py --gt_dir MOT15/aicup_gt/$date --ts_dir MOT15/aicup_ts/"${date}_${param}_${i}"/"$model" --mode single_cam --cam $cam
            done
        done
    done
done
...
do
    ...
    do
        ...
        do
            echo
            python calculate_ave_performance.py -f ts_result/"$model" -p "${param}" --start $start --end $end --step $step --cam $cam
        done
    done
done